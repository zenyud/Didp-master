...
class DataLoader:
    ...
    def __init__(self, args):
        ...
        self.__log_recorder = LogRecorder(job_id=self.__args.jobid,
    ...
    def __print_arguments(self):
        ...
        """
        LOG.debug("-------------------参数清单-------------------")
        LOG.debug("作业ID                       : {0}".format(self.__args.jobid))
        ...
        LOG.debug("Namenode url地址(大数据平台) : {0}".format(self.__args.url))
    ...
    def run(self):
        ...
        if not os.path.isfile(self.__args.srcfile):
            err_message = "DAT文件不存在[{0}]".format(self.__args.srcfile)
        ...
        if not os.path.isfile(self.__args.ddlfile):
            err_message = "DDL文件不存在[{0}]".format(self.__args.ddlfile)
            LOG.error(err_message)
            self.__log_recorder.record(job_starttime=start_time,
                                       job_status=1,
                                       err_message=err_message)
        ...
        try:
            target_module = importlib.import_module(
                "plugins.didp_{0}_plugin2".format(
        ...
        tabl_row = stat_table_record(target_db_info, self.__args.table)
        if tabl_row != -1:
            if file_row != 0 and (file_row - tabl_row)/file_row > self.__args.rjlimit:
                err_message = ("加载错误超过预设reject比例,"
                               "文件记录数[{0}]加载记录数[{1}]").format(file_row, tabl_row)
                LOG.error(err_message)
                self.__log_recorder.record(job_starttime=start_time,
                                           job_status=1,
                                           err_message=err_message)
            ...
            err_message = ("查询表记录数失败,表名[{0}]").format(self.__args.table)
            LOG.error(err_message)
            self.__log_recorder.record(job_starttime=start_time,
                                       job_status=1,
                                       err_message=err_message)
        ...
        # 更新元数据信息
        ret = DdlOperator().load_meta_ddl_direct(self.__args.tableid, self.__args.proid, ddl_info)
        if ret != 0:
            err_message = "解析DDL文件有误"
            LOG.error(err_message)
            self.__log_recorder.record(job_starttime=start_time,
                                       job_status=1,
                                       err_message=err_message)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-jobid",    required=True, help="作业ID")
    parser.add_argument("-proid",    required=True, help="项目版本ID")
    ...
    parser.add_argument("-nnurl",    required=False, help=u"[加载到Hive时必填]Namenode的IP地址(多个地址用分号分隔)")
